...
* `assets/shaders/slang/` — Slang sources. `assetconv --shaders` compiles every stage listed under `shaders` in `assets/manifest.json` to GLSL 450, column-major, into one `shaders.spnga`. `Shader` looks stages up by manifest name (`"pbr.vert"`), not by path.
