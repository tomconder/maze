...
static struct StringToLevelItem stringToLevelMap[] = { { "global", Level::Global },   { "debug", Level::Debug },
...
Logger::Logger(const std::string &id, base::LogStreamsReferenceMapPtr logStreamsReference)
    : m_id(id), m_typedConfigurations(nullptr), m_parentApplicationName(std::string()), m_isConfigured(false),
    ...
    for (; *format; ++format) {
        if (*format == base::consts::kFormatSpecifierChar) {
            ...
            case 'g':
                ...
                continue;
            case 'F': // AM/PM
                buf = base::utils::Str::addToBuff((tInfo->tm_hour >= 12) ? base::consts::kPm : base::consts::kAm, buf,
                                                  bufLim);
                continue;
            default:
                continue;
...
LogFormat::LogFormat(void)
    : m_level(Level::Unknown), m_userFormat(base::type::string_t()), m_format(base::type::string_t()),
      m_dateTimeFormat(std::string()), m_flags(0x0), m_currentUser(base::utils::OS::currentUser()),
...
LogFormat::LogFormat(const LogFormat &logFormat)
    : m_level(logFormat.m_level), m_userFormat(logFormat.m_userFormat), m_format(logFormat.m_format),
      m_dateTimeFormat(logFormat.m_dateTimeFormat), m_flags(logFormat.m_flags), m_currentUser(logFormat.m_currentUser),
...
void TypedConfigurations::insertFile(Level level, const std::string &fullFilename)
{
    ...
    std::string filePath =
        base::utils::File::extractPathFromFilename(resolvedFilename, base::consts::kFilePathSeparator);
...
Storage::Storage(const LogBuilderPtr &defaultLogBuilder)
    :
      ...
      m_registeredLoggers(new base::RegisteredLoggers(defaultLogBuilder)), m_flags(ELPP_DEFAULT_LOGGING_FLAGS),
