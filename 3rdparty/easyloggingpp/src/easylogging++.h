...
class DateTime : base::StaticClass
{
  public:
    ...
    /// microsecond.
    ///
    /// @detail For unix system it uses gettimeofday(timeval*, timezone*) and for Windows, a separate implementation is
    /// provided
...
class LogMessage
{
  public:
    LogMessage(Level level, const std::string &file, base::type::LineNumber line, const std::string &func,
        ...
        : m_level(level), m_file(file), m_line(line), m_func(func), m_verboseLevel(verboseLevel), m_logger(logger),
...
class Writer : base::NoCopy
{
  public:
    ...
    Writer(LogMessage *msg, base::DispatchAction dispatchAction = base::DispatchAction::NormalLog)
        : m_msg(msg), m_level(msg != nullptr ? msg->level() : Level::Unknown), m_line(0), m_logger(nullptr),
...
#else
template <typename T, typename... Args> inline void Logger::verbose(int, const char *, const T &, const Args &...)
{
    return;
}
template <typename T> inline void Logger::verbose(int, const T &)
{
    return;
}
...
#define LOGGER_LEVEL_WRITERS_DISABLED(FUNCTION_NAME, LOG_LEVEL)                                                        \
    ...
    template <typename T> inline void Logger::FUNCTION_NAME(const T &)                                                 \
    {                                                                                                                  \
        return;                                                                                                        \
    }
...
class PerformanceTrackingData
{
  public:
    ...
    explicit PerformanceTrackingData(DataType dataType)
        : m_performanceTracker(nullptr), m_dataType(dataType), m_firstCheckpoint(false), m_file(""), m_line(0),
          m_func("")
    {
    }
  ...
  public:
    ...
    PerformanceTracker(const PerformanceTracker &t)
        : m_blockName(t.m_blockName), m_timestampUnit(t.m_timestampUnit), m_loggerId(t.m_loggerId),
          ...
          m_lastCheckpointId(t.m_lastCheckpointId), m_enabled(t.m_enabled), m_startTime(t.m_startTime),
          m_endTime(t.m_endTime), m_lastCheckpointTime(t.m_lastCheckpointTime)
    {
    }
...
class Helpers : base::StaticClass
{
  public:
    ...
    template <typename T> static inline void uninstallPerformanceTrackingCallback(const std::string &id)
    {
...
#define TIMED_BLOCK(obj, blockName)                                                                                    \
    for (struct {                                                                                                      \
         ...
         } obj = { 0, el::base::type::PerformanceTrackerPtr(new el::base::PerformanceTracker(blockName,                \
                                                                                             ELPP_MIN_UNIT)) };        \
         obj.i < 1; ++obj.i)
