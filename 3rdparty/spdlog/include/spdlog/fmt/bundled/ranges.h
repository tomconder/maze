...
// C array overload
template <typename T, std::size_t N> auto range_begin(const T (&arr)[N]) -> const T *
{
    return arr;
}
template <typename T, std::size_t N> auto range_end(const T (&arr)[N]) -> const T *
{
    return arr + N;
}
...
{
    template <std::size_t... I> static std::true_type check2(index_sequence<I...>, integer_sequence<bool, (I == I)...>);
...
struct is_formattable_delayed : disjunction<is_formattable<uncvref_type<maybe_const_range<R>>, Char>,
                            ...
                            disjunction<is_formattable<T, Char>, detail::has_fallback_formatter<T, Char>>>::value>>
{
  private:
    ...
    template <class U> FMT_CONSTEXPR static auto maybe_set_debug_format(U &u, int) -> decltype(u.set_debug_format())
    {
        u.set_debug_format();
    }
  ...
  public:
    ...
    template <typename R, class FormatContext> auto format(R &&range, FormatContext &ctx) const -> decltype(ctx.out())
    {
...
struct range_default_formatter<
    K, R, Char, enable_if_t<(K == range_format::sequence || K == range_format::map || K == range_format::set)>>
{
...
struct range_format_kind : conditional_t<is_range<T, Char>::value, detail::range_format_kind_<T>,
