...
#ifdef _WIN32
template <> inline void write_escaped_path<char>(basic_memory_buffer<char> &quoted, const std::filesystem::path &p)
{
    auto s = p.u8string();
...
template <>
inline void
write_escaped_path<std::filesystem::path::value_type>(basic_memory_buffer<std::filesystem::path::value_type> &quoted,
...
template <typename Char> struct formatter<std::filesystem::path, Char> : formatter<basic_string_view<Char>>
{
...
template <typename T, typename U = void> struct is_variant_like_ : std::false_type
{
};
template <typename T> struct is_variant_like_<T, std::void_t<decltype(std::variant_size<T>::value)>> : std::true_type
{
};
