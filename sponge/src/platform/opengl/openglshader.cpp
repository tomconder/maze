...
OpenGLShader::OpenGLShader(const std::string &name, const std::string &vertexSource, const std::string &fragmentSource)
    ...
    SPONGE_CORE_INFO("Building program for shader = {}", name);
...
GLuint OpenGLShader::compileShader(const GLenum type, const std::string &source) {
    ...
    if (type == GL_VERTEX_SHADER) {
        SPONGE_CORE_INFO("Compiling vertex shader for shader = {}", name);
    } else {
        SPONGE_CORE_INFO("Compiling fragment shader for shader = {}", name);
    }
...
GLuint OpenGLShader::linkProgram(GLuint vs, GLuint fs) {
    ...
    SPONGE_CORE_INFO("Linking shaders: program = {}, shader = {}", id, name);
