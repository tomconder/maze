...
namespace sponge::platform::sdl {
    ...
    constexpr auto keyCodes = std::to_array({sponge::input::KeyCode::SpongeKey_W, sponge::input::KeyCode::SpongeKey_A,
    ...
    bool Engine::construct(const std::string_view name, const uint32_t width, const uint32_t height) {
        w = width;
        h = height;
        appName = name;
        ...
        if (std::isgreater(elapsedSeconds, CYCLE_TIME)) {
            ...
            while (SDL_PollEvent(&event) != 0) {
                ...
                if (imguiManager->isEventHandled() &&
                    (event.type == SDL_KEYUP || event.type == SDL_KEYDOWN || event.type == SDL_TEXTEDITING ||
            ...
            if (!imguiManager->isEventHandled()) {
                for (const auto &keycode: keyCodes) {
                    if (keyboard->isKeyPressed(keycode)) {
                        auto keyPressEvent = event::KeyPressedEvent{keycode, physicsTimer.getElapsedSeconds()};
    ...
    void Engine::setMouseVisible(const bool value) const {
        if (value) {
            SDL_WarpMouseInWindow(static_cast<SDL_Window *>(sdlWindow->getNativeWindow()),
                                  static_cast<int>(getWidth() / 2), static_cast<int>(getHeight() / 2));
    ...
    void Engine::processEvent(const SDL_Event &event, const double elapsedTime) {
        if (event.type == SDL_WINDOWEVENT && event.window.event == SDL_WINDOWEVENT_RESIZED) {
            ...
            auto keyEvent = event::KeyPressedEvent{input::Keyboard::mapScanCodeToKeyCode(event.key.keysym.scancode),
                                                   elapsedTime};
            onEvent(keyEvent);
        } else if (event.type == SDL_KEYUP) {
            auto keyEvent = event::KeyReleasedEvent{input::Keyboard::mapScanCodeToKeyCode(event.key.keysym.scancode)};
            onEvent(keyEvent);
        }
        ...
        if (event.type == SDL_MOUSEBUTTONDOWN) {
            ...
            auto mouseEvent = event::MouseMovedEvent{static_cast<float>(event.motion.xrel),
